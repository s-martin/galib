...
static char *fallbacks[] = {
	"*shell.title: gaview", "*background:	thistle4",
	"*canvas.background:	black", "*leftOffset:	3", "*rightOffset:	3",
	"*topOffset:	3", "*bottomOffset:	3", "*canvas.width:	400",
	"*canvas.height:	300", "*bestColor: green", "*populationColor1: yellow",
	"*populationColor2: blue", "*populationColor3: red",
	"*populationColor4: purple", "*populationColor5: cyan", "*ga: 2",
	"*genome: 0", "*function: 3", "*generationsPerStep: 10",

	// motif-specific fallbacks

	"*fontList:	-*-helvetica-bold-r-*-*-*-140-*-*-*-*-*-*",
	"*score.labelString:	score", "*stats.labelString:	stats",
	"*params.labelString:	params",

	// athena-specific fallbacks

	"*font:		-*-helvetica-bold-r-*-*-*-140-*-*-*-*-*-*",
	"*rewind.label:	reset", "*stop.label:		stop", "*step.label:		one",
...
#define Offset(field) (XtOffset(AppDataPtr, field))
	{"bestColor", XtCForeground, XtRPixel, sizeof(Pixel), Offset(bestcolor),
...
int main(int argc, char **argv)
{
	...
	if (theAppData.whichGA == 3)
	{
		...
		if (val > MAX_POPS)
		{
			...
			std::cerr << "this demo limits the number of populations to "
					  << MAX_POPS << "\n";
		}
	}
...
void DumpStatsCB(Widget, XtPointer cd, XtPointer)
{
	std::cerr << "\nstatistics are:\n"
			  << ((GAGeneticAlgorithm *)cd)->statistics() << "\n";
}

void DumpParamsCB(Widget, XtPointer cd, XtPointer)
{
	std::cerr << "\nparameters are:\n"
			  << ((GAGeneticAlgorithm *)cd)->parameters() << "\n";
}

void DumpScoreCB(Widget, XtPointer cd, XtPointer)
{
...
void DrawPopulation(Widget widget, const GAPopulation &pop, GC dotgc, GC bestgc)
{
	...
	if (theAppData.whichGenome == 1)
	{
		for (int i = 0; i < pop.size(); i++)
		{
			pts[i].x =
				originx + d / 2 +
				factor * ((GABin2DecGenome &)(pop.individual(i))).phenotype(0);
			pts[i].y =
				originy + d / 2 -
				factor * ((GABin2DecGenome &)(pop.individual(i))).phenotype(1);
		}
		xbest = originx + d / 2 +
				factor * ((GABin2DecGenome &)(pop.best())).phenotype(0);
		ybest = originy + d / 2 -
				factor * ((GABin2DecGenome &)(pop.best())).phenotype(1);
	}
	else
	{
		for (int i = 0; i < pop.size(); i++)
		{
			pts[i].x = originx + d / 2 +
					   factor * ((GARealGenome &)(pop.individual(i))).gene(0);
			pts[i].y = originy + d / 2 -
					   factor * ((GARealGenome &)(pop.individual(i))).gene(1);
		}
		xbest =
			originx + d / 2 + factor * ((GARealGenome &)(pop.best())).gene(0);
		ybest =
			originy + d / 2 - factor * ((GARealGenome &)(pop.best())).gene(1);
	}

	XDrawPoints(XtDisplay(widget), XtWindow(widget), dotgc, pts, npts,
				CoordModeOrigin);
	...
	XDrawPoint(XtDisplay(widget), XtWindow(widget), bestgc, xbest + 1, ybest);
	XDrawPoint(XtDisplay(widget), XtWindow(widget), bestgc, xbest, ybest + 1);
	XDrawPoint(XtDisplay(widget), XtWindow(widget), bestgc, xbest + 1,
			   ybest + 1);
	XDrawPoint(XtDisplay(widget), XtWindow(widget), bestgc, xbest + 1,
			   ybest - 1);
	XDrawPoint(XtDisplay(widget), XtWindow(widget), bestgc, xbest - 1,
...
Widget ConstructWidgets(Widget toplevel)
{
	...
	Pixel fg, bg;
	unsigned int depth;
	XtVaGetValues(form, XtNforeground, &fg, XtNbackground, &bg, XtNdepth,
				  &depth, NULL);
	...
	Widget rewind = XtVaCreateManagedWidget(
		"rewind", xmPushButtonWidgetClass, form, XmNleftAttachment,
		XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNlabelType,
		XmPIXMAP, XmNlabelPixmap, pix, NULL);
	...
	pix = XCreatePixmapFromBitmapData(
		XtDisplay(form), RootWindowOfScreen(XtScreen(form)),
		(char *)bm[bmFastForwardStop].bits, bm[bmFastForwardStop].width,
		bm[bmFastForwardStop].height, fg, bg, depth);
	...
	XtAddCallback(rewind, XmNactivateCallback, ResetCB, (XtPointer)&theAppData);
	XtAddCallback(stop, XmNactivateCallback, StopCB, (XtPointer)&theAppData);
	XtAddCallback(step, XmNactivateCallback, StepCB, (XtPointer)&theAppData);
	XtAddCallback(some, XmNactivateCallback, EvolveSomeCB,
				  (XtPointer)&theAppData);
	XtAddCallback(evolve, XmNactivateCallback, EvolveCB,
				  (XtPointer)&theAppData);

	Widget params = XtVaCreateManagedWidget(
		"params", xmPushButtonWidgetClass, form, XmNrightAttachment,
		XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, NULL);
	Widget stats = XtVaCreateManagedWidget(
		"stats", xmPushButtonWidgetClass, form, XmNrightAttachment,
		XmATTACH_WIDGET, XmNrightWidget, params, XmNbottomAttachment,
		XmATTACH_FORM, NULL);
	Widget score = XtVaCreateManagedWidget(
		"score", xmPushButtonWidgetClass, form, XmNrightAttachment,
		XmATTACH_WIDGET, XmNrightWidget, stats, XmNbottomAttachment,
		XmATTACH_FORM, NULL);
	...
	theAppData.counter = XtVaCreateManagedWidget(
		"counter", xmLabelWidgetClass, form, XmNtopAttachment, XmATTACH_FORM,
		XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
		XmNalignment, XmALIGNMENT_CENTER, NULL);
	Widget frame = XtVaCreateManagedWidget(
		"frame", xmFrameWidgetClass, form, XmNtopAttachment, XmATTACH_WIDGET,
		XmNtopWidget, theAppData.counter, XmNrightAttachment, XmATTACH_FORM,
		XmNleftAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_WIDGET,
		XmNbottomWidget, rewind, NULL);
	theAppData.canvas = XtVaCreateManagedWidget(
		"canvas", xmDrawingAreaWidgetClass, frame, NULL);
	...
	for (int kk = 0; kk < MAX_POPS; kk++)
	{
		...
		theAppData.dotgc[kk] = XCreateGC(XtDisplay(toplevel),
...
Widget ConstructWidgets(Widget toplevel)
{
	...
	theAppData.counter =
		XtVaCreateManagedWidget("counter", labelWidgetClass, form, NULL);

	theAppData.canvas = XtVaCreateManagedWidget(
		"canvas", widgetClass, form, XtNfromVert, theAppData.counter, NULL);
	XtAddEventHandler(theAppData.canvas, ExposureMask, False, ExposureEH,
					  (XtPointer)&theAppData);

	Widget ctrlbox = XtVaCreateManagedWidget("controls", boxWidgetClass, form,
											 XtNfromVert, theAppData.canvas,
											 XtNorientation, "vertical", NULL);
	Widget rewind =
