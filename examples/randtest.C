...
main(int argc, char **argv)
{
  ...
  for(i=1; i<argc; i++){
    if(strcmp("seed", argv[i]) == 0){
      if(++i >= argc) {
	 std::cerr << "You must enter a number when specifying a random seed.\n";
	exit(1);
      }
      else {
	seed = atoi(argv[i]);
...
// Tell us which RNG we're using...
  ...
  std::cout << "Using the " << GAGetRNG() << " random number generator (RNG).\n";
  ...
  if(dochisq) {
    ...
    std::cout << "  integer test (" << NCHI << "):  chi-squared should be within ";
	...
	chisq += ((double)f[i] - (double)NCHI / (double)RCHI) * 
      ...
      if(fabs(chisq - RCHI) > elimit) std::cout << "***failed***";
      std::cout << "\n";
    }
    std::cout.flush();
...
// check for values outside of bounds
  ...
  if(dobnds) {
    ...
    std::cout << "GARandomInt(-10,5) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
      vali = GARandomInt(-10,5);
      if(vali < -10 || vali > 5){ err += 1; }
    }
    if(err) std::cout << " " << err << " values out of bounds.\n";
    else std::cout << "ok\n";
    std::cout.flush();
    
     std::cerr << "  " << NUM_CHECKS << "  GARandomInt(0,3)...\n";
    std::cout << "GARandomInt(0,3) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
    ...
    std::cout << "GARandomInt(200,255) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
    ...
    std::cout << "GARandomFloat(-10,5) ..."; std::cout.flush();
    for(ii=0; ii<NUM_CHECKS; ii++){
      valf = GARandomFloat(-10,5);
      if(valf < -10 || valf > 5){ err += 1; lastf = valf; }
    }
    if(err) std::cout << " " << err << " values out of bounds (" << lastf << ").\n";
    else std::cout << "ok\n";
    std::cout.flush();
    
     std::cerr << "  " << NUM_CHECKS << "  GARandomFloat(0,3)...\n";
    std::cout << "GARandomFloat(0,3) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
    ...
    std::cout << "GARandomFloat(200,255) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
    ...
    std::cout << "GARandomDouble(-10,5) ..."; std::cout.flush();
    for(ii=0; ii<NUM_CHECKS; ii++){
      vald = GARandomDouble(-10,5);
      if(vald < -10 || vald > 5){ err += 1; lastd = vald; }
    }
    if(err) std::cout << " " << err << " values out of bounds (" << lastd << ").\n";
    else std::cout << "ok\n";
    std::cout.flush();
    
     std::cerr << "  " << NUM_CHECKS << "  GARandomDouble(0,3)...\n";
    std::cout << "GARandomDouble(0,3) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
    ...
    std::cout << "GARandomDouble(200,255) ..."; std::cout.flush();
    err = 0;
    for(ii=0; ii<NUM_CHECKS; ii++){
