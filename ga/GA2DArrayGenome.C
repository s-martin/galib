...
2DArrayGenome
---------------------------------------------------------------------------- */
template <class T> const char *GA2DArrayGenome<T>::className() const
{
	return "GA2DArrayGenome";
}
template <class T> int GA2DArrayGenome<T>::classID() const
{
	return GAID::ArrayGenome2D;
}
...
template <class T>
GA2DArrayGenome<T>::GA2DArrayGenome(const GA2DArrayGenome<T> &orig)
	: GAArray<T>(orig.sz)
{
	GA2DArrayGenome<T>::copy(orig);
}

template <class T> GA2DArrayGenome<T>::~GA2DArrayGenome() {}

template <class T> void GA2DArrayGenome<T>::copy(const GAGenome &orig)
{
	if (&orig == this)
		return;
...
template <class T> int GA2DArrayGenome<T>::resize(int w, int h)
{
...
template <class T> int GA2DArrayGenome<T>::read(std::istream &)
{
	GAErr(GA_LOC, className(), "read", GAError::OpUndef);
	return 1;
}

template <class T> int GA2DArrayGenome<T>::write(std::ostream &os) const
{
...
template <class T>
int GA2DArrayGenome<T>::resizeBehaviour(GAGenome::Dimension which,
...
template <class T>
void GA2DArrayGenome<T>::copy(const GA2DArrayGenome<T> &orig, unsigned int r,
							  unsigned int s, unsigned int x, unsigned int y,
							  unsigned int w, unsigned int h)
{
	if (w == 0 || x >= orig.nx || r >= nx || h == 0 || y >= orig.ny || s >= ny)
		return;
	if (x + w > orig.nx)
		w = orig.nx - x;
	if (y + h > orig.ny)
...
2DArrayAlleleGenome
---------------------------------------------------------------------------- */
template <class T> const char *GA2DArrayAlleleGenome<T>::className() const
{
	return "GA2DArrayAlleleGenome";
}
template <class T> int GA2DArrayAlleleGenome<T>::classID() const
{
	return GAID::ArrayAlleleGenome2D;
}

template <class T>
GA2DArrayAlleleGenome<T>::GA2DArrayAlleleGenome(unsigned int width,
												unsigned int height,
...
GA2DArrayAlleleGenome<T>::GA2DArrayAlleleGenome(
	const GA2DArrayAlleleGenome<T> &orig)
...
template <class T> GA2DArrayAlleleGenome<T>::~GA2DArrayAlleleGenome()
{
	delete[] aset;
}

template <class T>
GAGenome *GA2DArrayAlleleGenome<T>::clone(GAGenome::CloneMethod) const
{
	return new GA2DArrayAlleleGenome<T>(*this);
}

template <class T> void GA2DArrayAlleleGenome<T>::copy(const GAGenome &orig)
{
	if (&orig == this)
		return;
...
template <class T> int GA2DArrayAlleleGenome<T>::resize(int x, int y)
{
...
template <class T> int GA2DArrayAlleleGenome<T>::read(std::istream &is)
{
	return GA2DArrayGenome<T>::read(is);
}

template <class T> int GA2DArrayAlleleGenome<T>::write(std::ostream &os) const
{
...
template <class ARRAY_TYPE>
float GA2DArrayGenome<ARRAY_TYPE>::ElementComparator(const GAGenome &a,
													 const GAGenome &b)
{
...
template <class T>
int GA2DArrayGenome<T>::UniformCrossover(const GAGenome &p1, const GAGenome &p2,
...
template <class T>
int GA2DArrayGenome<T>::OnePointCrossover(const GAGenome &p1,
...
template <class T>
int GA2DArrayGenome<T>::EvenOddCrossover(const GAGenome &p1, const GAGenome &p2,
										 GAGenome *c1, GAGenome *c2)
{
	const GA2DArrayGenome<T> &mom = DYN_CAST(const GA2DArrayGenome<T> &, p1);
	const GA2DArrayGenome<T> &dad = DYN_CAST(const GA2DArrayGenome<T> &, p2);
			...
			sis.width() == mom.width() && sis.height() == mom.height())
		{
			...
			for (i = sis.width() - 1; i >= 0; i--)
			{
				for (j = sis.height() - 1; j >= 0; j--)
				{
					sis.gene(
						i, j,
						((count % 2 == 0) ? mom.gene(i, j) : dad.gene(i, j)));
					bro.gene(
						i, j,
						((count % 2 == 0) ? dad.gene(i, j) : mom.gene(i, j)));
					count++;
				}
			}
		}
