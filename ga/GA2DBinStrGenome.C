...
---------------------------------------------------------------------------- */
GA2DBinaryStringGenome::GA2DBinaryStringGenome(unsigned int width,
											   unsigned int height,
											   GAGenome::Evaluator f, void *u)
	: GABinaryString(width * height),
	  GAGenome(DEFAULT_2DBINSTR_INITIALIZER, DEFAULT_2DBINSTR_MUTATOR,
...
int GA2DBinaryStringGenome::read(std::istream &is)
{
	...
	if (is.eof() && ((j < ny) || // didn't get some lines
					 (i < nx && i != 0)))
...
int GA2DBinaryStringGenome::write(std::ostream &os) const
{
...
int GA2DBinaryStringGenome::resizeBehaviour(Dimension which, unsigned int lower,
											unsigned int upper)
{
	if (upper < lower)
	{
...
void GA2DBinaryStringGenome::set(unsigned int x, unsigned int y, unsigned int w,
								 unsigned int h)
{
	if (x + w > nx)
	{
								   ...
								   unsigned int w, unsigned int h) const
{
	...
	for (unsigned int j = 0; j < h; j++)
	{
		eq += static_cast<unsigned int>(GABinaryString::equal(orig, (y + j) * nx + x,
									(srcy + j) * nx + srcx, w));
	}
...
float GA2DBinaryStringGenome::BitComparator(const GAGenome &a,
											const GAGenome &b)
{
...
int GA2DBinaryStringGenome::UniformCrossover(const GAGenome &p1,
											 ...
											 GAGenome *c2)
{
		...
		else
		{
			...
			for (i = 0; i < minx; i++)
			{
				for (j = 0; j < miny; j++)
				{
					sis.gene(
						i, j,
						(mask[i * miny + j] != 0 ? mom.gene(i, j) : dad.gene(i, j)));
				}
			}
			...
			for (i = 0; i < minx; i++)
			{
				for (j = 0; j < miny; j++)
				{
					bro.gene(
						i, j,
						(mask[i * miny + j] != 0 ? dad.gene(i, j) : mom.gene(i, j)));
				}
			}
		}
	...
	{
		GA2DBinaryStringGenome &sis =
			(c1 != nullptr ? DYN_CAST(GA2DBinaryStringGenome &, *c1)
			...
			sis.width() == mom.width() && sis.height() == mom.height())
		{
			for (i = sis.width() - 1; i >= 0; i--)
			{
				...
				{
					sis.gene(i, j,
							 (GARandomBit() != 0 ? mom.gene(i, j) : dad.gene(i, j)));
				}
			}
		}
		else
...
int GA2DBinaryStringGenome::OnePointCrossover(const GAGenome &p1,
											  const GAGenome &p2, GAGenome *c1,
											  GAGenome *c2)
{
	const GA2DBinaryStringGenome &mom =
...
int GA2DBinaryStringGenome::EvenOddCrossover(const GAGenome &p1,
											 const GAGenome &p2, GAGenome *c1,
											 GAGenome *c2)
{
	const GA2DBinaryStringGenome &mom =
