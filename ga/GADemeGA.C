...
GADemeGA &GADemeGA::operator=(const GADemeGA &orig)
{
	if (&orig != this)
	{
		copy(orig);
...
void GADemeGA::copy(const GAGeneticAlgorithm &g)
{
...
void GADemeGA::objectiveData(int i, const GAEvalData &v)
{
	if (i == ALL)
	{
		for (unsigned int ii = 0; ii < npop; ii++)
...
const GAPopulation &GADemeGA::population(int i, const GAPopulation &p)
{
	if (i == ALL)
	{
		for (unsigned int ii = 0; ii < npop; ii++)
...
int GADemeGA::populationSize(int i, unsigned int value)
{
	if (value < 1)
	{
...
int GADemeGA::nReplacement(int i, unsigned int value)
{
	if (i == ALL)
	{
		for (unsigned int ii = 0; ii < npop; ii++)
	...
	if (static_cast<unsigned int>(tmppop->size()) < value)
	{
		tmppop->size(value);
	}
	return value;
...
GAScalingScheme &GADemeGA::scaling(int i, const GAScalingScheme &s)
{
	if (i == ALL)
	{
		for (unsigned int ii = 0; ii < npop; ii++)
...
GASelectionScheme &GADemeGA::selector(int i, const GASelectionScheme &s)
{
	if (i == ALL)
	{
		for (unsigned int ii = 0; ii < npop; ii++)
...
int GADemeGA::nPopulations(unsigned int n)
{
	if (n < 1 || n == npop)
	{
		return npop;
...
void GADemeGA::initialize(unsigned int seed)
{
	GARandomSeed(seed);
...
void GADemeGA::step()
{
	...
	for (unsigned int ii = 0; ii < npop; ii++)
	{
		...
		{ // takes care of odd population
			...
			{
				pstats[ii].numcro +=
					(*scross)(*mom, *dad, &tmppop->individual(i),
			...
			pstats[ii].nummut +=
				(mut = tmppop->individual(i).mutate(pMutation()));
			if (mut > 0)
			{
				c1 = 1;
			}
			pstats[ii].nummut +=
				(mut = tmppop->individual(i + 1).mutate(pMutation()));
			if (mut > 0)
			{
				c2 = 1;
			}
		...
		{
			tmppop->replace(
				deme[ii]->remove(GAPopulation::WORST, GAPopulation::SCALED), i);
		}
	...
	stats.numsel = stats.numcro = stats.nummut = stats.numrep = stats.numeval =
		0;
