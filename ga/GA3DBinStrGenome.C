...
---------------------------------------------------------------------------- */
GA3DBinaryStringGenome::GA3DBinaryStringGenome(unsigned int width,
											   unsigned int height,
											   unsigned int depth,
											   GAGenome::Evaluator f, void *u)
	: GABinaryString(width * height * depth),
	  GAGenome(DEFAULT_3DBINSTR_INITIALIZER, DEFAULT_3DBINSTR_MUTATOR,
...
int GA3DBinaryStringGenome::read(std::istream &is)
{
	...
	if (is.eof() && ((k < nz) || // didn't get some lines
...
int GA3DBinaryStringGenome::write(std::ostream &os) const
{
...
int GA3DBinaryStringGenome::resizeBehaviour(Dimension which, unsigned int lower,
											unsigned int upper)
{
	if (upper < lower)
	{
...
void GA3DBinaryStringGenome::copy(const GA3DBinaryStringGenome &orig,
...
void GA3DBinaryStringGenome::set(unsigned int x, unsigned int y, unsigned int z,
...
void GA3DBinaryStringGenome::unset(unsigned int x, unsigned int y,
...
void GA3DBinaryStringGenome::randomize(unsigned int x, unsigned int y,
									   unsigned int z, unsigned int w,
									   unsigned int h, unsigned int d)
{
	if (x + w > nx)
...
void GA3DBinaryStringGenome::move(unsigned int x, unsigned int y,
								   ...
								   unsigned int d) const
{
	...
	for (unsigned int k = 0; k < d; k++)
	{
		for (unsigned int j = 0; j < h; j++)
		{
			eq += static_cast<unsigned int>(GABinaryString::equal(
...
float GA3DBinaryStringGenome::BitComparator(const GAGenome &a,
											const GAGenome &b)
{
	const GA3DBinaryStringGenome &sis =
		DYN_CAST(const GA3DBinaryStringGenome &, a);
...
int GA3DBinaryStringGenome::UniformCrossover(const GAGenome &p1,
											 ...
											 GAGenome *c2)
{
	...
	else if ((c1 != nullptr) || (c2 != nullptr))
	{
		GA3DBinaryStringGenome &sis =
			(c1 != nullptr ? DYN_CAST(GA3DBinaryStringGenome &, *c1)
...
// It might break!
int GA3DBinaryStringGenome::EvenOddCrossover(const GAGenome &p1,
											 const GAGenome &p2, GAGenome *c1,
											 GAGenome *c2)
{
	const GA3DBinaryStringGenome &mom =
...
int GA3DBinaryStringGenome::OnePointCrossover(const GAGenome &p1,
											  const GAGenome &p2, GAGenome *c1,
											  GAGenome *c2)
{
	const GA3DBinaryStringGenome &mom =
