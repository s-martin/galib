...
1DArrayGenome
---------------------------------------------------------------------------- */
template <class T> const char *GA1DArrayGenome<T>::className() const
{
	return "GA1DArrayGenome";
}
template <class T> int GA1DArrayGenome<T>::classID() const
{
	return GAID::ArrayGenome;
}
...
template <class T>
GA1DArrayGenome<T>::GA1DArrayGenome(unsigned int length, GAGenome::Evaluator f,
									void *u)
	: GAArray<T>(length),
...
template <class T>
GA1DArrayGenome<T>::GA1DArrayGenome(const GA1DArrayGenome<T> &orig)
...
template <class T> int GA1DArrayGenome<T>::write(std::ostream &os) const
{
...
template <class T>
int GA1DArrayGenome<T>::resizeBehaviour(unsigned int lower, unsigned int upper)
{
	if (upper < lower)
	{
...
---------------------------------------------------------------------------- */
template <class T> const char *GA1DArrayAlleleGenome<T>::className() const
{
	return "GA1DArrayAlleleGenome";
}
template <class T> int GA1DArrayAlleleGenome<T>::classID() const
{
	return GAID::ArrayAlleleGenome;
}

template <class T>
GA1DArrayAlleleGenome<T>::GA1DArrayAlleleGenome(unsigned int length,
...
template <class T>
GA1DArrayAlleleGenome<T>::GA1DArrayAlleleGenome(const GAAlleleSetArray<T> &sa,
												GAGenome::Evaluator f, void *u)
...
GA1DArrayAlleleGenome<T>::GA1DArrayAlleleGenome(
	const GA1DArrayAlleleGenome<T> &orig)
...
template <class T>
GAGenome *GA1DArrayAlleleGenome<T>::clone(GAGenome::CloneMethod) const
{
...
template <class T> int GA1DArrayAlleleGenome<T>::write(std::ostream &os) const
{
...
void GA1DArrayAlleleGenome<ARRAY_TYPE>::UniformInitializer(GAGenome &c)
{
...
template <class ARRAY_TYPE>
void GA1DArrayAlleleGenome<ARRAY_TYPE>::OrderedInitializer(GAGenome &c)
{
	GA1DArrayAlleleGenome<ARRAY_TYPE> &child =
		DYN_CAST(GA1DArrayAlleleGenome<ARRAY_TYPE> &, c);
	child.resize(GAGenome::ANY_SIZE); // let chrom resize if it can
...
template <class ARRAY_TYPE>
float GA1DArrayGenome<ARRAY_TYPE>::ElementComparator(const GAGenome &a,
													 const GAGenome &b)
{
...
template <class T>
int GA1DArrayGenome<T>::UniformCrossover(const GAGenome &p1, const GAGenome &p2,
...
template <class T>
int GA1DArrayGenome<T>::OnePointCrossover(const GAGenome &p1,
										  ...
										  GAGenome *c2)
{
	...
	if (c1 && c2)
	{
		...
		{
			if (mom.length() != dad.length() || sis.length() != bro.length() ||
				sis.length() != mom.length())
			{
...
template <class T>
int GA1DArrayGenome<T>::TwoPointCrossover(const GAGenome &p1,
										  const GAGenome &p2, GAGenome *c1,
										  GAGenome *c2)
{
	const GA1DArrayGenome<T> &mom = DYN_CAST(const GA1DArrayGenome<T> &, p1);
...
template <class T>
int GA1DArrayGenome<T>::EvenOddCrossover(const GAGenome &p1, const GAGenome &p2,
										 GAGenome *c1, GAGenome *c2)
{
	const GA1DArrayGenome<T> &mom = DYN_CAST(const GA1DArrayGenome<T> &, p1);
	const GA1DArrayGenome<T> &dad = DYN_CAST(const GA1DArrayGenome<T> &, p2);
...
template <class T>
int GA1DArrayGenome<T>::PartialMatchCrossover(const GAGenome &p1,
											  const GAGenome &p2, GAGenome *c1,
											  GAGenome *c2)
{
	const GA1DArrayGenome<T> &mom = DYN_CAST(const GA1DArrayGenome<T> &, p1);
	...
	if (c1 && c2)
	{
		...
		{
			for (j = 0; j < sis.length() - 1 && sis.gene(j) != dad.gene(index);
				 j++)
				;
			sis.swap(i, j);
		}
		...
		for (i = a, index = a; i < b; i++, index++)
		{
			for (j = 0; j < bro.length() - 1 && bro.gene(j) != mom.gene(index);
				 j++)
				;
			bro.swap(i, j);
		}
	...
	else if (c1 || c2)
	{
		...
		{
			for (j = 0;
				 j < sis.length() - 1 && sis.gene(j) != parent2->gene(index);
				 j++)
				;
			sis.swap(i, j);
		}
...
template <class T>
int GA1DArrayIsHole(const GA1DArrayGenome<T> &c, const GA1DArrayGenome<T> &dad,
...
template <class T>
int GA1DArrayGenome<T>::OrderCrossover(const GAGenome &p1, const GAGenome &p2,
									   GAGenome *c1, GAGenome *c2)
{
	const GA1DArrayGenome<T> &mom = DYN_CAST(const GA1DArrayGenome<T> &, p1);
	const GA1DArrayGenome<T> &dad = DYN_CAST(const GA1DArrayGenome<T> &, p2);
...
template <class T>
int GA1DArrayGenome<T>::CycleCrossover(const GAGenome &p1, const GAGenome &p2,
									   GAGenome *c1, GAGenome *c2)
{
	const GA1DArrayGenome<T> &mom = DYN_CAST(const GA1DArrayGenome<T> &, p1);
	const GA1DArrayGenome<T> &dad = DYN_CAST(const GA1DArrayGenome<T> &, p2);
