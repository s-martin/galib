...
template <class T> GAList<T>::~GAList()
{
	while (hd)
...
template <class T> void GAList<T>::copy(const GAList<T> &orig)
{
	while (hd)
		delete GAListBASE::remove(DYN_CAST(GANode<T> *, hd));
...
template <class T> T *GAList<T>::remove()
{
...
// list.
template <class T> GAList<T> *GAList<T>::clone(unsigned int i) const
{
...
template <class T> int GAList<T>::destroy()
{
...
template <class T> int GAList<T>::swap(unsigned int a, unsigned int b)
{
