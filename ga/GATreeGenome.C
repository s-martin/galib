...
template <class T> void _tt(std::ostream &os, GANode<T> *n)
{
	if (!n)
		return;
...
template <class T> int GATreeGenome<T>::write(std::ostream &os) const
{
...
template <class T>
int GATreeGenome<T>::DestructiveMutator(GAGenome &c, float pmut)
{
...
//   After the mutation the iterator is left at the root of the tree.
template <class T> int GATreeGenome<T>::SwapNodeMutator(GAGenome &c, float pmut)
{
	GATreeGenome<T> &child = DYN_CAST(GATreeGenome<T> &, c);
	int n, i;
	if (pmut <= 0.0)
...
template <class T>
float GATreeGenome<T>::TopologyComparator(const GAGenome &a, const GAGenome &b)
{
	if (&a == &b)
		return 0;
...
template <class T>
int GATreeGenome<T>::OnePointCrossover(const GAGenome &p1, const GAGenome &p2,
