...
GAGenome &GAUniformSelector::select() const
{
...
GAGenome &GARouletteWheelSelector::select() const
{
	...
	return pop->individual(
		lower, (which == SCALED ? GAPopulation::SCALED : GAPopulation::RAW));
}
...
void GARouletteWheelSelector::update()
{
	if (pop->size() != n)
	{
		delete[] psum;
	...
	if (which == GASelectionScheme::RAW)
	{
		...
		else
		{
			GAErr(GA_LOC, className(), "update - objective",
				  "objective scores are not strictly negative or strictly "
				  "positive",
	...
	else
	{
		...
		{
			GAErr(
				GA_LOC, className(), "update - fitness",
...
GAGenome &GASRSSelector::select() const
{
	return pop->individual(
		choices[GARandomInt(0, pop->size() - 1)],
		(which == SCALED ? GAPopulation::SCALED : GAPopulation::RAW));
}
...
---------------------------------------------------------------------------- */
#if USE_DS_SELECTOR == 1
GAGenome &GADSSelector::select() const
{
	return pop->individual(
		choices[GARandomInt(0, pop->size() - 1)],
		(which == SCALED ? GAPopulation::SCALED : GAPopulation::RAW));
