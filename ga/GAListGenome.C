...
template <class T> const char *GAListGenome<T>::className() const
{
	return "GAListGenome";
}
template <class T> int GAListGenome<T>::classID() const
{
	return GAID::ListGenome;
}
...
template <class T> GAListGenome<T>::~GAListGenome() {}
...
template <class T> void GAListGenome<T>::copy(const GAGenome &orig)
{
	if (&orig == this)
		return;
...
template <class T> int GAListGenome<T>::write(std::ostream &os) const
{
...
template <class T> bool GAListGenome<T>::equal(const GAGenome &c) const
{
	if (this == &c)
		return true;
...
template <class T>
int GAListGenome<T>::DestructiveMutator(GAGenome &c, float pmut)
{
...
//   After the mutation the iterator is left at the head of the list.
template <class T> int GAListGenome<T>::SwapMutator(GAGenome &c, float pmut)
{
	GAListGenome<T> &child = DYN_CAST(GAListGenome<T> &, c);
	int n, i;
	if (pmut <= 0.0)
...
template <class T>
float GAListGenome<T>::NodeComparator(const GAGenome &a, const GAGenome &b)
{
	if (&a == &b)
		return 0;
...
template <class T>
int GAListGenome<T>::OnePointCrossover(const GAGenome &p1, const GAGenome &p2,
...
template <class T>
int GAListGenome<T>::PartialMatchCrossover(const GAGenome &p1,
...
template <class T>
int GAListIsHole(const GAListGenome<T> &child, const GAListGenome<T> &parent,
...
template <class T>
int GAListGenome<T>::OrderCrossover(const GAGenome &p1, const GAGenome &p2,
									GAGenome *c1, GAGenome *c2)
{
	const GAListGenome<T> &mom = DYN_CAST(const GAListGenome<T> &, p1);
	const GAListGenome<T> &dad = DYN_CAST(const GAListGenome<T> &, p2);
			...
			if (*sis.current() != *diter.current())
			{
				...
				for (j = i + 1; j < b; j++)
					if (*siter.next() == *diter.current())
					{
						sis.swap(i, j);
						sis.warp(
							siter); // move iterator back to previous location
						break;
					}
			}
		}
			...
			if (*bro.current() != *miter.current())
			{
				...
				for (j = i + 1; j < b; j++)
					if (*biter.next() == *miter.current())
					{
						bro.swap(i, j);
						bro.warp(
							biter); // move iterator back to previous location
						break;
					}
			}
		}
...
template <class T>
int GAListGenome<T>::CycleCrossover(const GAGenome &p1, const GAGenome &p2,
									GAGenome *c1, GAGenome *c2)
{
	const GAListGenome<T> &mom = DYN_CAST(const GAListGenome<T> &, p1);
	const GAListGenome<T> &dad = DYN_CAST(const GAListGenome<T> &, p2);
	...
	if (c1)
	{
		...
		// Now fill in the rest of the sis with dad's contents that we didn't
		// use in the cycle.
		sis.head();
		diter.head();
	...
	if (c2)
	{
		...
		// Now fill in the rest of the bro with dad's contents that we didn't
		// use in the cycle.
		bro.head();
		miter.head();
