...
GA1DBinaryStringGenome::GA1DBinaryStringGenome(unsigned int len,
											   GAGenome::Evaluator f, void *u)
	: GABinaryString(len),
	  GAGenome(DEFAULT_1DBINSTR_INITIALIZER, DEFAULT_1DBINSTR_MUTATOR,
			   DEFAULT_1DBINSTR_COMPARATOR)
{
	evaluator(f);
	userData(u);
	crossover(
		DEFAULT_1DBINSTR_CROSSOVER); // assign the default sexual crossover
	nx = minX = maxX = 0;
	resize(len);
}
...
int GA1DBinaryStringGenome::read(std::istream &is)
{
	static char c;
	unsigned int i = 0;
...
int GA1DBinaryStringGenome::resizeBehaviour(unsigned int lower,
											unsigned int upper)
{
	if (upper < lower)
	{
...
bool GA1DBinaryStringGenome::equal(const GA1DBinaryStringGenome &c,
...
int GA1DBinaryStringGenome::FlipMutator(GAGenome &c, float pmut)
{
	...
	{ // only flip the number of bits we need to flip
		...
		{
			int i = GARandomInt(0, child.length() -
									   1); // the index of the bit to flip
			child.gene(i, ((child.gene(i) == 0) ? 1 : 0));
		}
	}
...
// object)
float GA1DBinaryStringGenome::BitComparator(const GAGenome &a,
											const GAGenome &b)
{
	const GA1DBinaryStringGenome &sis =
		DYN_CAST(const GA1DBinaryStringGenome &, a);
...
int GA1DBinaryStringGenome::UniformCrossover(const GAGenome &p1,
											 ...
											 GAGenome *c2)
{
	...
	{
		GA1DBinaryStringGenome &sis =
			(c1 != nullptr ? DYN_CAST(GA1DBinaryStringGenome &, *c1)
...
int GA1DBinaryStringGenome::OnePointCrossover(const GAGenome &p1,
											  const GAGenome &p2, GAGenome *c1,
											  GAGenome *c2)
{
	const GA1DBinaryStringGenome &mom =
...
int GA1DBinaryStringGenome::TwoPointCrossover(const GAGenome &p1,
											  const GAGenome &p2, GAGenome *c1,
											  GAGenome *c2)
{
	const GA1DBinaryStringGenome &mom =
...
int GA1DBinaryStringGenome::EvenOddCrossover(const GAGenome &p1,
											 const GAGenome &p2, GAGenome *c1,
											 GAGenome *c2)
{
	const GA1DBinaryStringGenome &mom =
