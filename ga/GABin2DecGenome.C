...
void GABin2DecPhenotype::add(unsigned int nb, float min, float max)
{
	if (core->n + 1 > core->N)
	{
		core->N += core->csz;
...
// requested.
float GABin2DecGenome::phenotype(unsigned int n) const
{
	...
	float val = 0.0;
	decode(val, &(data[ptype->offset(n)]), ptype->length(n), ptype->min(n),
		   ptype->max(n));
	return val;
}
...
float GABin2DecGenome::phenotype(unsigned int n, float val)
{
	if (n >= ptype->nPhenotypes())
	{
		GAErr(GA_LOC, className(), "phenotype", gaErrBadPhenotypeID);
