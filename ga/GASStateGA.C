...
GASteadyStateGA::GASteadyStateGA(const GAGenome &c) : GAGeneticAlgorithm(c)
{
	...
	float n =
		((pRepl * (float)pop->size() < 1) ? 1 : pRepl * (float)pop->size());
...
const GAPopulation &GASteadyStateGA::population(const GAPopulation &p)
{
	...
	{
		if (nRepl > (unsigned int)(pop->size()))
			nRepl = pop->size();
		if (nRepl < 1)
			nRepl = 1;
	}
...
int GASteadyStateGA::populationSize(unsigned int value)
{
	...
	{ // if we're using nrepl, be sure in valid range
		if (nRepl > value)
		{ // clip to new population size
			nRepl = value;
			tmpPop->size(nRepl);
		}
	}
...
float GASteadyStateGA::pReplacement(float value)
{
	...
	float n =
		((value * (float)pop->size() < 1) ? 1 : value * (float)pop->size());
	nRepl = (unsigned int)n;
	params.set(gaNnReplacement, (unsigned int)nRepl);
...
void GASteadyStateGA::initialize(unsigned int seed)
{
	GARandomSeed(seed);
...
void GASteadyStateGA::step()
{
	...
	// Replace the worst genomes in the main population with all of the
	// individuals we just created.  Notice that we invoke the population's add
	// member with a genome pointer rather than reference.  This way we don't
