...
int GATreeBASE::swapnode(GANodeBASE *a, GANodeBASE *b)
{
	...
	{ // a & b are not adjacent
		...
		{ // a and b are not adjacent nor parent-child
			a->parent = bparent;
			a->child = bchild;
			b->parent = aparent;
			b->child = achild;
	...
	if (rt == a)
		rt = b; // this only works if they're in the same tree!
	else if (rt == b)
		rt = a;
...
// wise we return a 0.
int GATreeBASE::ancestral(unsigned int i, unsigned int j) const
{
...
int GATreeIterBASE::nsiblings(GANodeBASE *c)
{
	GANodeBASE *tmp = c;
	int n = 1;
...
// we go up from there.
GANodeBASE *GATreeIterBASE::warp(unsigned int x)
{
	unsigned int w = 0;
...
int GATreeIterBASE::size(GANodeBASE *n) { return (_GATreeSize(n)); }

// Return the depth of the tree from the specified node on down.
int GATreeIterBASE::depth(GANodeBASE *n) { return (_GATreeDepth(n)); }
