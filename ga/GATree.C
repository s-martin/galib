...
//template <class T> void GATree<T>::copy(const GATree<T> &orig)
//{
//	_GATreeDestroy(DYN_CAST(GANode<T> *, rt));
...
//template <class T> int GATree<T>::swaptree(GATree<T> *t)
//{
//	GANodeBASE *tmp;
...
//
//		t->csz = 1;
//		t->cdpth = 1; // remember to flag the changes in t!
//		csz = 1;
//		cdpth = 1;
//	}
//	else if (t->iter.node)
//	{ // iter.node is NULL, so we have no root
//		if (!rt)
...
//	}
//	else if (iter.node)
//	{ // t's iter is NULL, so it has no root
//		if (!t->rt)
...
//// swap.
//template <class T> int GATree<T>::swaptree(unsigned int a, unsigned int b)
//{
