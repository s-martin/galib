...
class GA1DBinaryStringGenome : public GABinaryString, public GAGenome
{
  ...
  public:
	...
	{
		_evaluated = false;
		return ((bit(x) == value) ? value : bit(x, value));
	}
...
inline void GA1DBinaryStringGenome::copy(const GA1DBinaryStringGenome &orig,
...
inline void GA1DBinaryStringGenome::move(unsigned int x, unsigned int srcx,
										 unsigned int l)
{
	if (srcx + l > nx)
	{
