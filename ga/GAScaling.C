...
void GANoScaling::evaluate(const GAPopulation &p)
{
...
void GALinearScaling::evaluate(const GAPopulation &p)
{
	// Here we calculate the slope and intercept using the multiplier and
	...
	double delta, a, b;
	if (pave == pmax)
	{ // no scaling - population is all the same
		a = 1.0;
		b = 0.0;
	}
	else if (pmin > (static_cast<double>(c) * pave - pmax) /
	...
	// allowed with this kind of scaling, so check for negative values.  If we
	// get a negative value, dump an error message then set all of the scores to
	// 0.
	...
	for (int i = 0; i < p.size(); i++)
	{
		...
		p.individual(i).fitness(
			static_cast<float>(f)); // might lose information here!
	}
}
...
float GALinearScaling::multiplier(float fm)
{
	if (fm <= 1.0)
	{
...
void GASigmaTruncationScaling::evaluate(const GAPopulation &p)
{
	for (int i = 0; i < p.size(); i++)
	{
		double f = static_cast<double>(p.individual(i).score()) -
				   static_cast<double>(p.ave());
...
// equal to zero.
float GASigmaTruncationScaling::multiplier(float fm)
{
	if (fm < 0.0)
	{
...
void GAPowerLawScaling::evaluate(const GAPopulation &p)
{
	for (int i = 0; i < p.size(); i++)
	{
		double f = p.individual(i).score();
...
void GASharing::evaluate(const GAPopulation &p)
{
	...
	if (_minmax == 0)
	{
		...
		else
		{
			mm = ((p.order() == GAPopulation::HIGH_IS_BEST)
					  ? GAGeneticAlgorithm::MAXIMIZE
					  : GAGeneticAlgorithm::MINIMIZE);
		}
	}
	else
	{
...
void GASharing::copy(const GAScalingScheme &arg)
{
	if (&arg == this)
	{
		return;
